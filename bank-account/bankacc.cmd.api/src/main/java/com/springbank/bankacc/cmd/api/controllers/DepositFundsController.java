...
public class DepositFundsController {
    ...
    public ResponseEntity<BaseResponse> depositFunds(@PathVariable(value = "id") String id, @Valid @RequestBody DepositFundsCommand command) {
        try {
            ...
            return new ResponseEntity<>(new BaseResponse("Funds successfully deposited!"), HttpStatus.CREATED);
        } catch (Exception e) {
            var safeErrorMessage = "Error wile processing request to deposit funds into bak account for id - ".concat(id);
